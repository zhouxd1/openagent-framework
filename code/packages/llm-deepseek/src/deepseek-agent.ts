 ...
 * 
 * An agent implementation that uses DeepSeek's chat completion API
...
import { DeepSeekAgentConfig, Message, ToolCall, ToolDefinition } from './types';

const logger = createLogger('DeepSeekAgent');

/**
 * DeepSeek Agent
 * 
 * An intelligent agent that uses DeepSeek's chat completion API with support
...
export class DeepSeekAgent extends BaseAgent {
  ...
  /**
   * Create a new DeepSeek agent instance
   * 
   * @param config - Agent configuration
   */
  constructor(config: DeepSeekAgentConfig) {
    // Create AgentConfig from DeepSeekAgentConfig
    const agentConfig: AgentConfig = {
      id: config.id || `agent-${generateId()}`,
      name: config.name || 'DeepSeek Agent',
      provider: 'deepseek',
      model: config.model,
    ...
    this.agentLogger = createLogger('DeepSeekAgent', { agentId: this.id, agentName: this.name });
    ...
    this.agentLogger.info('DeepSeek agent created', {
      model: config.model,
   ...
   * 2. Converts tools to OpenAI function format (compatible with DeepSeek)
   * 3. Calls the DeepSeek API with tool support
  ...
  async run(input: string, context?: AgentContext): Promise<AgentResponse> {
    ...
    try {
      ...
      // Convert tools to OpenAI format (compatible with DeepSeek)
      const tools = this.getTools().length > 0 ? this.convertTools(this.getTools()) : undefined;

      // Execute with tools support
      const response = await this.deepseekProvider.executeWithTools(
        {
          messages,
          tools,
  ...
  /**
   * Convert internal tools to OpenAI function format (compatible with DeepSeek)
   */
  ...
  async *stream(
    ...
    try {
      for await (const chunk of this.deepseekProvider.stream({
        messages,
        tools,
        temperature: this.config.temperature,
        maxTokens: this.config.maxTokens,
